function formatDuration(seconds: number): string {
    ...
    if (y) rez.push(format(y, "year"));
    if (d) rez.push(format(d, "day"));
    if (h) rez.push(format(h, "hour"));
    if (m) rez.push(format(m, "minute"));
    if (s) rez.push(format(s, "second"));

    let returns = rez.join(", ") || "now";
    let last = returns.lastIndexOf(",");
    return (last < 0) ? returns : returns.slice(0, last) + " and" + returns.slice(last + 1);

}

/*комбинации лет, дней, часов, минут и секунд.*/
